    ...
    public class Driver
    {
        ...
        public static void ReadWrite32BitValue()
        {
            using (SerialPort port = new SerialPort("COM3"))
            {
                ...
                //ushort[] registers = master.ReadHoldingRegisters(slaveId, startAddress, Ndata);
                try
                {
                    //registers = master.ReadInputRegisters(slaveId, startAddress, Ndata);
                    registers = master.ReadHoldingRegisters(slaveId, startAddress, Ndata);
                }
                catch (Exception ex)
                {
                ...
                float ValF = ModbusUtility.GetSingle(registers[0], registers[1]);

                int value = ModbusUtility.GetInt32(registers[1], registers[0]);
                int value2 = ModbusUtility.GetInt32(registers[0], registers[1]);

                //long value = ModbusUtility.GetLong(registers[0], registers[1], registers[2], registers[3]);
