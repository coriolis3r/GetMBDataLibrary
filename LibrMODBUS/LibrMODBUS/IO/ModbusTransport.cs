...
namespace Modbus.IO
{
    ...
    public abstract class ModbusTransport : IDisposable
    {
            ...
            do
            {
                ...
                catch (Exception e)
                {
                    if (e is FormatException ||
                        ...
                        e is IOException)
                    {
                        //System.Diagnostics.Trace.WriteLine("Se generó una excepción en Modbus Transport: " + "\n");
                        _logger.WarnFormat("{0}, {1} retries remaining - {2}", e.GetType().Name, _retries - attempt + 1, e);

                        if (attempt++ > _retries)
                        {
                            //response = ReadResponseNull<T>();
                            //success = true;
                            throw;
                        }
                    }
        ...
        internal abstract IModbusMessage ReadResponseNull<T>() where T : IModbusMessage, new();
